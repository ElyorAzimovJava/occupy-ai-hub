    ...
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
        <aside className="space-y-4 rounded-2xl border border-border bg-card p-5">
          <div><div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Joy holati</div><div className="mt-3 space-y-2">{(["available","occupied","reserved","disabled"] as SlotStatus[]).map((s) => { const labels: Record<SlotStatus,string> = { available: "Bo'sh", occupied: "Band", reserved: "Zahira", disabled: "Yopiq" }; const initials: Record<SlotStatus,string> = { available: "B", occupied: "B", reserved: "Z", disabled: "Y" }; return (<button key={s} onClick={() => setPaint(s)} className={`flex w-full items-center justify-between rounded-xl border px-3 py-2 text-sm transition ${paint===s?"border-primary bg-primary/10 text-primary":"border-border hover:bg-muted/40"}`}><span className="flex items-center gap-2"><Brush className="h-3.5 w-3.5"/>{labels[s]}</span><Badged tone={s==="available"?"success":s==="occupied"?"danger":s==="reserved"?"warning":"default"}>{initials[s]}</Badged></button>); })}</div></div>
